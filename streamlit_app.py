        ...
        if df is None:
            ...
            for i in range(top_num):
                if prediction[i*2+1]>0:
                    ...
                    other_products_predicted = other_products_predicted or True
    if other_products_predicted:
        st.markdown(f"<font color='red' size=3 >*</font><font color='green' size=3 >{other_products_label}</font> includes {', '.join(other_products)}", unsafe_allow_html=True)x